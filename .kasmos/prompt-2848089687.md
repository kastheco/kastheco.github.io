...
### important
- `blog/src/lib/markdown.ts:259` — `remarkObsidianCallouts()` silently drops callout body content. When `[!TYPE]` and body share a single remark paragraph (no blank line separating type indicator from body), `firstText.value = titleText` overwrites the entire text node — including the body text that follows the `[!TYPE]` marker on subsequent lines. Confirmed in built HTML: all three demo callouts render as empty or near-empty title divs with no content div at all. Fix: after extracting `titleText` from the first text node, split the remaining body text (content after the first `\n`) into a new paragraph node and append it to `node.children` before slicing into `callout-content`.

### minor
- `blog/package.json:18` — `remark-obsidian-callout` is listed as a dependency but is never imported or used anywhere (the coder correctly chose a local visitor and documented why). Remove it with `npm uninstall remark-obsidian-callout` and commit the updated lockfile.

### self-fixed (no action needed)
- `blog/src/pages/posts/[slug].astro:5` — removed unused `getPostPath` import flagged by `astro check` (committed directly as reviewer self-fix).

verdict: changes required

## remediation
- Fix `remarkObsidianCallouts` in `blog/src/lib/markdown.ts` to preserve body text. After setting `firstText.value = titleText`, check whether the original text node contained a `\n`. If it did, split everything after the first newline into a new text node and wrap it in a paragraph that becomes a child of the content div. Then `node.children.slice(1)` picks up the remaining blockquote paragraphs as before.
- Run `npm uninstall remark-obsidian-callout` in `blog/` and commit updated `package-lock.json`.
